...
import { editMessageSchema, EditMessageSchemaType } from "@/lib/zod-schema";
import { useAction } from "next-safe-action/hooks";
import { editMessage } from "@/actions/job.actions";
import { MessagesData } from "@/lib/queries";
...
export function EditMessageButton({
  ...
  const form = useForm<EditMessageSchemaType>({
    resolver: zodResolver(editMessageSchema),
    defaultValues: {
      id: message.id,
      jobId: message.jobId,
      employeeName: message.employeeName,
      employeeRole: message.employeeRole,
      messageTone: message.messageTone,
      customInstructions: message.customInstructions || "",
    },
  });

  const { execute, isExecuting } = useAction(editMessage, {
    onSuccess: () => {
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Edit Personalized Message
          </DialogTitle>
          <DialogDescription>
            Update the personalized email message for a {companyName} employee
